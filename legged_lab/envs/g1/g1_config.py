...
class G1RewardCfg(RewardCfg):
    ...
    feet_to_near_humanoid = RewTerm(func=mdp.feet_to_near_humanoid, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot", body_names=[".*ankle_roll.*"]), "threshold": 0.2})
    ...
    joint_deviation_arms = RewTerm(func=mdp.joint_deviation_l1, weight=-0.2, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*waist_yaw.*", ".*_shoulder_roll.*", ".*_shoulder_yaw.*", ".*_wrist.*"])})
    joint_deviation_waist = RewTerm(func=mdp.joint_deviation_l1, weight=-0.4, params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*waist_pitch.*", ".*waist_roll.*"])})
...
@configclass
class G1RoughEnvCfg(BaseEnvCfg):
    scene: G1SceneCfg = G1SceneCfg(
